...
B) BlockingQueue/TransferQueue
    ...
    - LinkedBlockingQueue       // 由链表组成；有界但默认值很大（Integer.MAX_VALUE）(但可以指定CAPACITY)；不可以指定公平/非公平 (固定非公平); 读写锁锁分离
...
//Concurrent：主要实现的是线程安全
C) Concurrent:
    - ConcurrentLinkedQueue: 是使用非阻塞的方式实现的基于链接节点的无界的线程安全队列，性能非常好  (注意：并没有ConcurreentArrayQueue)
